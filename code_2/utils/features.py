...
def pack_audio_files_to_hdf5(args):
    ...
    audio_names = config.file_names
    audio_paths = config.file_paths
    ...
    meta_dict = {
        ...
        'target': np.array([int(audio_name.split('-')[-1].split('.')[0]) for audio_name in audio_names]),
        'fold': np.array([int(audio_name.split('-')[0]) for audio_name in audio_names])
    }

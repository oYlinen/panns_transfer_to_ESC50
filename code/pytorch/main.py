...
from data_generator import ESC50Data_, EvaluateSampler, collate_fn
...
def train(args):
    ...
    training = df[df['fold'] != 5]
    validation = df[df['fold'] == 5]

    train_flag = True
    valid_flag = False

    train_data = ESC50Data_('C:/Users/imspr/ESC-50-master/audio/', training, 'filename', 'category')
    valid_data = ESC50Data_('C:/Users/imspr/ESC-50-master/audio/', validation, 'filename', 'category')


    # Data loader
    train_loader = torch.utils.data.DataLoader(dataset=train_data,
        num_workers=num_workers, batch_size=32*2, shuffle=True)

    validate_loader = torch.utils.data.DataLoader(dataset=valid_data,
        num_workers=num_workers, batch_size=32)
